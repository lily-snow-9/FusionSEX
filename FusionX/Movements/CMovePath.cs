...
namespace FusionX.Movements
{
    class CMovePath : CMove
    {
        ...
        public bool MT_Direction = false;
        ...
        public override void move()
        {
	        ...
	        while(true)
	        {
	            ...
	            {
	                MT_FlagBranch=false;
	                bool flag=mtMove(hoPtr.hoAdRunHeader.rhMT_MoveStep);
	        ...
	        if (MT_Direction==false)
	        {
                ...
                if (number<MT_Movement.mtNumber)					//; Dernier mouvement?
                {
                    ...
                    if (MT_GotoNode!=null)
                    {
                        ...
                        {
                            if (String.Compare(MT_GotoNode, MT_Movement.steps[number].mdName, StringComparison.OrdinalIgnoreCase) == 0)
                            {
        ...
        void mtMessages()
        {
            ...
            if (hoPtr.hoMark1 == hoPtr.hoAdRunHeader.rhLoopCount)
            {
                ...
                hoPtr.hoAdRunHeader.rhEvtProg.handle_Event(hoPtr, (-20 << 16) | (((int)hoPtr.hoType) & 0xFFFF));	    // CNDL_EXTPATHNODE
                hoPtr.hoAdRunHeader.rhEvtProg.handle_Event(hoPtr, (-35 << 16) | (((int)hoPtr.hoType) & 0xFFFF));	    // CNDL_EXTPATHNODENAME
            }
            if (hoPtr.hoMark2 == hoPtr.hoAdRunHeader.rhLoopCount)
            {
                hoPtr.hoAdRunHeader.rhEvtProg.rhCurParam0 = 0;
                hoPtr.hoAdRunHeader.rhEvtProg.handle_Event(hoPtr, (-21 << 16) | (((int)hoPtr.hoType) & 0xFFFF));   // CNDL_EXTENDPATH
            } 
        }
        // Fin du mouvement
        // ~~~~~~~~~~~~~~~~
        ...
        public void mtBranchNode(String pName)
        {
            ...
            for (number = 0; number < MT_Movement.mtNumber; number++)
            {
                if (MT_Movement.steps[number].mdName != null)
                {
                    if (String.Compare(pName, MT_Movement.steps[number].mdName, StringComparison.OrdinalIgnoreCase) == 0)
                    {
                        if (MT_Direction == false)
                        {
                            // En avant
