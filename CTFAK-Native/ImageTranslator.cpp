	...
	DllExport void ReadPoint(char* result, int width, int height, int alpha,int size, char* imageData,int transparentColor)
	{
            ...
            if (alpha)
            {
                ...
                for (int y = 0; y < height; y++)
                {
                    for (int x = 0; x < width; x++)
                    {
                        char alphaData = imageData[alphaSize];
                        result[(y * aStride) + (x * 4) + 3] = alphaData;
                        if(alphaData==0)
                        {
                            result[(y * aStride) + (x * 4) + 2]=0;
                            result[(y * aStride) + (x * 4) + 1]=0;
                            result[(y * aStride) + (x * 4) + 0]=0;
                        }
